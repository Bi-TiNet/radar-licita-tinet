...
2. Executar `supabase/schema.sql` no SQL Editor do projeto novo. Criar apenas a conta autorizada em Authentication e inserir o e-mail dela em `public.radar_access` conforme o comentário no fim do SQL. Desabilitar novos cadastros públicos. Não usar a `service_role` no site.
3. Configurar no GitHub, como *Actions secrets*, `RADAR_SUPABASE_URL` e `RADAR_SUPABASE_SERVICE_ROLE_KEY`. Manter `RADAR_NOTIFICATIONS_ENABLED` ausente ou diferente de `1`. Definir `RADAR_CLOUD_READY=true` como *Actions variable* apenas depois de confirmar os segredos. Executar `Coleta Radar Licita` manualmente e verificar uma linha `success` em `sync_runs`, conferindo a amostra de licitações no Supabase.
4. Criar um site Netlify separado com este repositório, usando `netlify.toml`. Definir apenas `VITE_SUPABASE_URL` e `VITE_SUPABASE_ANON_KEY` como variáveis de build. Confirmar que a página pede login, que uma conta não autorizada não vê dados e que a conta liberada consegue ver os registros.
...
6. Fazer um teste controlado de entrega para cada um dos dois e-mails e dos dois números. Só então definir `RADAR_NOTIFICATIONS_ENABLED=1` no GitHub.
